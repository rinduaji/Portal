...
class DaftarCCM extends CI_Controller {

	function __construct(){
		parent::__construct();		
		$this->load->model('m_daftar_ccm');
	...
	public function index()
	{
		...
		$jumlah_data = $this->m_daftar_ccm->jumlah_data($user_id,$jb_agent[0],$data['cari'],$data['date_awal'],$data['date_akhir'], $data['status']);

		$config['base_url'] = base_url('index.php/DaftarCCM/index');
        ...
        $data['data_list_ccm'] = $this->m_daftar_ccm->data_list_ccm($user_id,$jb_agent[0],$config['per_page'],$data['start'],$data['cari'],$data['date_awal'],$data['date_akhir'], $data['status']);
        // $data['nama_list_ccm_kategori'] = $this->m_daftar_ccm->nama_list_ccm_kategori();
		...
		$data['cek_verifikasi'] = $this->m_daftar_ccm->cek_verifikasi($user_id);
		...
		$this->load->view('admin/daftar_ccm_view',$data);
    ...
    public function ApproveAgent($id) {
		...
		$cekJabatanIdCCM = $this->m_daftar_ccm->cekJabatanIdCCM($id)->user3;
		$cekId = $this->m_daftar_ccm->cekJabatanIdCCM($id)->spv;
		// print_r($cekId);
        $jb_agent = explode(" ",$cekJabatanIdCCM);
        if(isset($jb_agent[0])) {
            if($jb_agent[0] == 'AGENT' || $jb_agent[0] == 'INPUTER' || $jb_agent[0] == "SUPPORT" || $jb_agent[0] == "SOO") {
                $cekjabatan = ($jb_agent[0]);
                // if($jb_agent[2] == 'QCO') {
                //     $cekjabatan = ($jb_agent[0].' '.$jb_agent[1].' '.$jb_agent[2]); 
                // }
		...
		$data['hasil_jabatan'] = $cekjabatan;
		$data['data_nama_groupmail'] = $this->m_daftar_ccm->getNamaGroupmail($cekId,$jb_user[0]);
		$data['data_approve_agent'] = $this->m_daftar_ccm->data_ccm_approve_agent($id);
    ...
    public function ApproveTL($id) {
		...
		$data['hasil_jabatan'] = $cekjabatan;
		$data['data_nama_groupmail'] = $this->m_daftar_ccm->getNamaGroupmail($cekId,$jb_user[0]);
		$data['data_approve_tl'] = $this->m_daftar_ccm->data_ccm_approve_tl($id);
    ...
    public function ApproveSpv($id) {
		...
		$data['hasil_jabatan'] = $cekjabatan;
		$data['data_nama_groupmail'] = $this->m_daftar_ccm->getNamaGroupmail($cekId,$jb_user[0]);
		$data['data_approve_spv'] = $this->m_daftar_ccm->data_ccm_approve_spv($id);
	...
	public function ApproveManager($id) {
		...
		$data['hasil_jabatan'] = $cekjabatan;
		$data['data_nama_groupmail'] = $this->m_daftar_ccm->getNamaGroupmail($cekId,$jb_user[0]);
		$data['data_approve_manager'] = $this->m_daftar_ccm->data_ccm_approve_manager($id);
    ...
    public function Verifikasi($id) {
		...
		$data['hasil_jabatan'] = $cekjabatan;
		$data['data_nama_groupmail'] = $this->m_daftar_ccm->getNamaGroupmail($cekId,$jb_user[0]);
		$data['data_approve_verifikasi'] = $this->m_daftar_ccm->data_ccm_approve_verifikasi($id);
	...
	public function updateApproveAgent() {
        ...
        if ($komitmen != "" AND $lup_agn != "") {
			$this->m_daftar_ccm->updateApprove($id,$data);
			$this->m_daftar_ccm->updateApproveNotif($id,$data_notif);
			$this->session->set_flashdata('success', 'Data Berhasil diubah');
        }
		else {
			$this->session->set_flashdata('gagal', 'Data Gagal diubah');
		}
		redirect(site_url('DaftarCCM/index'));
	}
	...
	public function updateApproveManager() {
        ...
        if ($lup_mgr != "") {
			$this->m_daftar_ccm->updateApproveManager($id,$data);
			// $this->m_daftar_ccm->updateApproveManagerNotif($id,$data_notif);
			$this->session->set_flashdata('success', 'Data Berhasil diubah');
        }
		else {
			$this->session->set_flashdata('gagal', 'Data Gagal diubah');
	...
	public function edit(){
        ...
        if ($validation->run()) {
			$this->m_daftar_ccm->update($id,$data);
			$this->session->set_flashdata('success', 'Data Berhasil diubah');
        }
		else {
			$this->session->set_flashdata('gagal', 'Data Gagal diubah');
	...
	public function get_by_id($id) {
		$data = $this->m_daftar_ccm->get_id($id);
		echo json_encode($data);
	}

	public function hapus() {
		$id= $this->input->post('id');
		if($this->m_daftar_ccm->delete($id)) {
