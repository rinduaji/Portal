...
class M_daftar_ccm extends CI_Model{	
    ...
    function data_list_ccm($user_id,$jb,$limit,$start,$cari = null, $date_awal = null, $date_akhir = null, $status = null){	
        $this->db->select('a.id, b.username, b.name, f.user3, c.user2, a.tgl_mulai, a.tgl_akhir, a.kode, a.status, a.level, a.spv, b.user_id as id_anak, b.`name` as nama_anak, c.user_id as id_tl, c.user2 as nama_tl, d.user_id as id_spv, d.user2 as nama_spv, e.user_id as id_manager, e.user2 as nama_manager');
        $this->db->from($this->table3.' AS a');
        $this->db->join($this->table.' AS b', 'a.no_karywn = b.user_id');
        ...
        // $this->db->join($this->table2.' AS a', $this->table3.'.spv = a.id');
          if($jb == "AGENT" || $jb == "INPUTER" || $jb == "SOO" || $jb == "SUPPORT") {
